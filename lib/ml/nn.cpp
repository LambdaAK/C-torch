...
namespace ml {
    ...
    // Change back_prop to not return anything and to be internal somehow
    // Accumlate gradients until zero_grad() called
    // Have optimizer that requires optimizer.step()

    NN_SGD::NN_SGD(std::vector<std::pair<std::shared_ptr<Matrix>, std::shared_ptr<Matrix>>> p, float lr, size_t batch_size) : params(p), lr(lr), batch_size(batch_size) {}

    void NN_SGD::zero_grad() {
        for (auto& p : params) {
            (*p.second) = 0.0 * (*p.second); // zero the gradients
        }
    }
    ...
    void NN_SGD::step() {
        for (auto& p : params) {
            // (*p.second).print();
            (*p.first) = (*p.first) - lr * ((1.0/batch_size) * (*p.second)); // take a step, average gradient over batch size
        }
    }
